...
function OnboardingForm() {
  ...
  const [form, setForm] = useState({
    displayName: "", company: "", phone: "", role: "", companySize: "", currency: "USD",
  });
  ...
  async function finish() {
    ...
    setApiError("");
    try {
      // POST /api/v1/users/me/onboarding — saves all profile fields, marks onboardingComplete = true
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 32, justifyContent: "center" }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg, #4f7eff 0%, #38d9a9 100%)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 20px rgba(79,126,255,0.3)" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 28, alignItems: "center" }}>
          {stepInfo.map((s, i) => (
            <div key={s.num} style={{ display: "flex", alignItems: "center", flex: i < stepInfo.length - 1 ? 1 : undefined }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 20, padding: "32px 28px", boxShadow: "var(--shadow)" }}>
          ...
          {step === 1 && (
            <div>
              ...
              <p style={{ fontSize: 13, color: "var(--text-2)", marginBottom: 24 }}>
                {provider !== "email" ? `Signed in with ${provider} · ` : ""}We need a few details to personalise your experience.
              </p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  <input className={inputCls} placeholder="How should we call you?" value={form.displayName} onChange={e => set("displayName", e.target.value)} />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-2)", display: "block", marginBottom: 6 }}>
                    <Phone size={11} style={{ marginRight: 5, verticalAlign: "middle" }} />Phone <span style={{ color: "var(--text-3)", fontWeight: 400 }}>(optional)</span>
                  </label>
              ...
              <button onClick={() => setStep(2)} disabled={!form.displayName} style={{
                width: "100%", marginTop: 28, padding: "13px", borderRadius: 10, border: "none",
                background: form.displayName ? "var(--accent)" : "var(--border)",
                color: form.displayName ? "#fff" : "var(--text-3)",
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 28 }}>
                <button onClick={() => setStep(1)} style={{ padding: "13px 24px", borderRadius: 10, border: "1px solid var(--border)", background: "transparent", color: "var(--text-2)", fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Back</button>
                <button onClick={() => setStep(3)} style={{ flex: 1, padding: "13px", borderRadius: 10, border: "none", background: "var(--accent)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Syne, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  Continue <ArrowRight size={15} />
                </button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  <select value={form.currency} onChange={e => set("currency", e.target.value)}
                    style={{ width: "100%", background: "var(--bg-card2)", border: "1px solid var(--border)", borderRadius: 10, padding: "12px 14px", fontSize: 14, color: "var(--text-1)", fontFamily: "inherit", outline: "none", appearance: "none", cursor: "pointer" }}>
                ...
                <div style={{ background: "var(--bg-card2)", border: "1px solid var(--border-dim)", borderRadius: 12, padding: "16px" }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>Your Profile Summary</p>
                  {[["Name", form.displayName],["Company", form.company || "—"],["Role", form.role || "—"],["Team", form.companySize || "—"],["Currency", form.currency]].map(([k, v]) => (
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 28 }}>
                <button onClick={() => setStep(2)} style={{ padding: "13px 24px", borderRadius: 10, border: "1px solid var(--border)", background: "transparent", color: "var(--text-2)", fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Back</button>
                <button onClick={finish} disabled={loading} style={{
                  flex: 1, padding: "13px", borderRadius: 10, border: "none",
                  background: loading ? "var(--border)" : "var(--green)", color: loading ? "var(--text-3)" : "#fff",
                  fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer",
                  fontFamily: "Syne, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                }}>
                  {loading
                    ? <><span style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block", animation: "spin 0.7s linear infinite" }} /> Setting up workspace…</>
                    : <><Check size={15} /> Enter Accord</>}
                </button>
              </div>
            </div>
          )}
