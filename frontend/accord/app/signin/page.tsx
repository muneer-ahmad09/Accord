...
import { Eye, EyeOff, Gauge, ArrowRight } from "lucide-react";
...
export default function SignIn() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // signIn sets the JWT + user in context
      router.push("/");
    } catch (err) {
  ...
  // ── OAuth sign-in ─────────────────────────────────────────────────────────
  // Spring Boot exposes OAuth2 login at /oauth2/authorization/{provider}
  // After the provider redirects back, Spring issues our JWT and redirects
  // to /api/v1/auth/oauth/success?token=<jwt>&new=<bool>
  // ConditionalShell / OAuthCallback page reads the ?token param and stores it.
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 20, padding: "36px 32px", boxShadow: "var(--shadow)" }}>
          <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: 24, fontWeight: 800, color: "var(--text-1)", marginBottom: 6 }}>Welcome back</h1>
          <p style={{ fontSize: 13, color: "var(--text-2)", marginBottom: 28 }}>Sign in to your Accord account</p>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
            {[
              ...
              <button key={id} onClick={() => handleOAuth(id)} style={{
                ...
                padding: "11px 16px", background: "var(--bg-card2)", border: "1px solid var(--border)",
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                ...
                <Link href="#" style={{ fontSize: 11, color: "var(--accent)", textDecoration: "none" }}>Forgot password?</Link>
              </div>
